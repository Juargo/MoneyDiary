...
describe('calcularEstadoBucket — Necesidades (SC-N-01..06)', () => {
  it('SC-N-06: 0n → Verde', () => {
    expect(calcularEstadoBucket(Bucket.Necesidades, 0n)).toBe(
      EstadoSemaforo.Verde,
    );
  });

  it('SC-N-01: 5000n → Verde (exact lower green boundary, greener side inclusive)', () => {
    expect(calcularEstadoBucket(Bucket.Necesidades, 5000n)).toBe(
      EstadoSemaforo.Verde,
    );
  });

  it('SC-N-02: 5001n → Amarillo (one bp above green boundary)', () => {
    expect(calcularEstadoBucket(Bucket.Necesidades, 5001n)).toBe(
      EstadoSemaforo.Amarillo,
    );
  });

  it('SC-N-03: 6000n → Amarillo (exact upper Amarillo boundary, greener side inclusive)', () => {
    expect(calcularEstadoBucket(Bucket.Necesidades, 6000n)).toBe(
      EstadoSemaforo.Amarillo,
    );
  });

  it('SC-N-04: 6001n → Rojo (one bp above Amarillo boundary)', () => {
    expect(calcularEstadoBucket(Bucket.Necesidades, 6001n)).toBe(
      EstadoSemaforo.Rojo,
    );
  });

  it('SC-N-05: 8000n → Rojo (deep Rojo)', () => {
    expect(calcularEstadoBucket(Bucket.Necesidades, 8000n)).toBe(
      EstadoSemaforo.Rojo,
    );
  });
});
...
describe('calcularEstadoBucket — Deseos (SC-D-01..04)', () => {
  it('SC-D-01: 3000n → Verde (exact green boundary)', () => {
    expect(calcularEstadoBucket(Bucket.Deseos, 3000n)).toBe(
      EstadoSemaforo.Verde,
    );
  });

  it('SC-D-02: 3001n → Amarillo (one bp above green boundary)', () => {
    expect(calcularEstadoBucket(Bucket.Deseos, 3001n)).toBe(
      EstadoSemaforo.Amarillo,
    );
  });

  it('SC-D-03: 4000n → Amarillo (exact upper Amarillo boundary)', () => {
    expect(calcularEstadoBucket(Bucket.Deseos, 4000n)).toBe(
      EstadoSemaforo.Amarillo,
    );
  });

  it('SC-D-04: 4001n → Rojo (one bp above Amarillo boundary)', () => {
    expect(calcularEstadoBucket(Bucket.Deseos, 4001n)).toBe(
      EstadoSemaforo.Rojo,
    );
  });
});
...
describe('calcularEstadoBucket — Ahorro bidirectional (SC-A-01..10)', () => {
  ...
  it('SC-A-05: 1000n → Amarillo (exact lower Amarillo boundary)', () => {
    expect(calcularEstadoBucket(Bucket.Ahorro, 1000n)).toBe(
      EstadoSemaforo.Amarillo,
    );
  });

  it('SC-A-02: 1999n → Amarillo (one bp below lower Verde boundary)', () => {
    expect(calcularEstadoBucket(Bucket.Ahorro, 1999n)).toBe(
      EstadoSemaforo.Amarillo,
    );
  });

  it('SC-A-01: 2000n → Verde (exact lower Verde boundary)', () => {
    expect(calcularEstadoBucket(Bucket.Ahorro, 2000n)).toBe(
      EstadoSemaforo.Verde,
    );
  });

  it('interior: 3000n → Verde', () => {
    expect(calcularEstadoBucket(Bucket.Ahorro, 3000n)).toBe(
      EstadoSemaforo.Verde,
    );
  });

  it('SC-A-03: 4000n → Verde (exact upper Verde boundary)', () => {
    expect(calcularEstadoBucket(Bucket.Ahorro, 4000n)).toBe(
      EstadoSemaforo.Verde,
    );
  });

  it('SC-A-04: 4001n → Amarillo (one bp above upper Verde boundary)', () => {
    expect(calcularEstadoBucket(Bucket.Ahorro, 4001n)).toBe(
      EstadoSemaforo.Amarillo,
    );
  });

  it('SC-A-07: 5000n → Amarillo (exact upper Amarillo boundary)', () => {
    expect(calcularEstadoBucket(Bucket.Ahorro, 5000n)).toBe(
      EstadoSemaforo.Amarillo,
    );
  });

  it('SC-A-08: 5001n → Rojo (one bp above upper Amarillo boundary)', () => {
    expect(calcularEstadoBucket(Bucket.Ahorro, 5001n)).toBe(
      EstadoSemaforo.Rojo,
    );
  });

  it('SC-A-10: 10000n → Rojo (extreme over-saving 100%)', () => {
    expect(calcularEstadoBucket(Bucket.Ahorro, 10000n)).toBe(
      EstadoSemaforo.Rojo,
    );
  });
});
