 ...
 * (US-040, design.md §5.3). Mirrors `aCatalogoHttpError`'s shape — pero NO
 * es una reutilización: unión distinta, traductor propio (design.md §5.3).
 * Un class ⇒ exactamente un status ⇒ exactamente un `code`.
 *
 * El guard `const _exhaustive: never = error` es la garantía de compilación:
 * agregar una variante a `ActualizarPerfilError` sin mapearla acá DEJA DE
 * COMPILAR. En la otra dirección, agregar `EmailNoDisponibleError` a esa
 * unión también deja de compilar — esa es la prueba de que nunca alcanza el
 * boundary HTTP (D-01, colapsado siempre a `PerfilRechazadoError`).
 */
export function aPerfilHttpError(error: ActualizarPerfilError): {
  status: number;
  code: string;
  message: string;
} {
