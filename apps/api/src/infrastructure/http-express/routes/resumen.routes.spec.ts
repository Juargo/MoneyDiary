  ...
  describe('GET /api/resumen', () => {
    it('200 con el DTO y llama al use case con userId + periodo', async () => {
      ...
      const res = await request(probeApp(mes, { execute: vi.fn() })).get(
        '/api/resumen?periodo=2026-07',
      );

      expect(res.status).toBe(200);
      expect(res.body.periodo).toBe('2026-07');
      expect(mes.execute).toHaveBeenCalledWith({
        userId: 'user-x',
        periodo: '2026-07',
      });
    });
    ...
    it('400 scrubbeado si el periodo es inválido (nunca refleja el input)', async () => {
      ...
      const res = await request(probeApp(mes, { execute: vi.fn() })).get(
        '/api/resumen?periodo=input-malo',
      );
  ...
  describe('GET /api/resumen/anual', () => {
    it('200 con el DTO y llama con userId + anio', async () => {
      const anual = {
        execute: vi
          .fn()
          .mockResolvedValue(Result.ok({ resumenAnual: RESUMEN_ANUAL_OK })),
      };
      const res = await request(probeApp({ execute: vi.fn() }, anual)).get(
        '/api/resumen/anual?anio=2026',
      );

      expect(res.status).toBe(200);
      expect(res.body.anio).toBe(2026);
      expect(anual.execute).toHaveBeenCalledWith({
        userId: 'user-x',
        anio: '2026',
      });
    });
    ...
    it('400 scrubbeado si el anio es inválido', async () => {
      const anual = {
        execute: vi
          .fn()
          .mockResolvedValue(Result.fail(new AnioInvalidoError(99999))),
      };
      const res = await request(probeApp({ execute: vi.fn() }, anual)).get(
        '/api/resumen/anual?anio=99999',
      );
