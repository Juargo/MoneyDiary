...
describe('BancoEstadoPdfStrategy', () => {
  const strategy = new BancoEstadoPdfStrategy();

  it('matches: reconoce la cartola real de BancoEstado (PDF-01)', async () => {
    const tokens = await tokensPagina1('bancoestado-cartola-test.pdf');
    expect(strategy.matches(tokens)).toBe(true);
  });

  it('extract: retorna BancoEstado, CuentaRUT y el número de cuenta del encabezado', async () => {
    const tokens = await tokensPagina1('bancoestado-cartola-test.pdf');
