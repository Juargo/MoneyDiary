...
- Terminology (binding domain vocabulary): **cartola** (bank statement file), **ingesta** (one import event, PROCESADA/FALLIDA), **semáforo** (worst-of-3-buckets monthly verdict), **movimiento** (transaction), **patrones** (auto-categorization rules), **buckets** Necesidades/Deseos/Ahorro plus "Sin grupo ni categoría" (`bucketId IS NULL` — no grupo AND therefore no categoría; distinct from the synthetic "Sin categoría" group that appears INSIDE a bucket for `categoriaId IS NULL`). UI deliberately relabels `Deseos` → **"Gustos"** and `SinCategoria` → **"Sin grupo ni categoría"** (via `ETIQUETA_BUCKET`); the API keeps `Deseos`/`SinCategoria`.
