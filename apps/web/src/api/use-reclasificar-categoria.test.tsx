...
describe('useReclasificarCategoria', () => {
  ...
  it('en onSuccess invalida las queries de resumen, detalle-bucket, detalle-bucket-mes y resumen-anual (WCAT-04)', async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      status: 200,
      json: () => Promise.resolve(validDto),
  ...
  it('sin periodo, invalida ["resumen", "actual"], ["detalle-bucket", bucket, "actual"] y ["detalle-bucket-mes", bucket, "actual"] (mismo fallback que los hooks de lectura)', async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      status: 200,
      json: () => Promise.resolve(validDto),
