...
describe('fetchMe', () => {
  ...
  it('en éxito acepta un usuario demo (email:null, esDemo:true)', async () => {
    mockFetchOnce({
      ok: true,
      status: 200,
      json: () => Promise.resolve(validDemoMeDto),
    });
  ...
  it('mapea un body 2xx sin esDemo (forma pre-demo obsoleta) a {tag: "parse"}', async () => {
    mockFetchOnce({
      ...
      status: 200,
      json: () =>
        Promise.resolve({ userId: 'user-1', email: 'usuario@moneydiary.cl' }),
    });
  ...
  it('mapea {esDemo:false, email:null} (usuario real sin email) a {tag: "parse"} — fail-closed', async () => {
    mockFetchOnce({
      ...
      status: 200,
      json: () =>
        Promise.resolve({ userId: 'user-1', email: null, esDemo: false }),
    });
