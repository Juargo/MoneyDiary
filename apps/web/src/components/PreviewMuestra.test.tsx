...
describe('PreviewMuestra', () => {
  ...
  it('slices to the given cantidad — cantidad=10 on a 15-row muestra shows exactly 10 rows', () => {
    ...
    render(
      <PreviewMuestra muestra={muestra} banco="BancoEstado" totalFilasDatos={15} cantidad={10} onCantidadChange={vi.fn()} />,
    )
  ...
  it('cantidad=25 on a 12-row muestra shows all 12 rows without padding or error', () => {
    ...
    render(
      <PreviewMuestra muestra={muestra} banco="BancoEstado" totalFilasDatos={12} cantidad={25} onCantidadChange={vi.fn()} />,
    )
  ...
  it('exposes the current cantidad via aria-pressed on the selector buttons (a11y, not color-only)', () => {
    ...
    render(
      <PreviewMuestra muestra={muestra} banco="BancoEstado" totalFilasDatos={1} cantidad={25} onCantidadChange={vi.fn()} />,
    )
  ...
  it('the selector group has an accessible name (associated label)', () => {
    ...
    expect(screen.getByRole('group', { name: /filas a mostrar/i })).toBeInTheDocument()
  })
})
