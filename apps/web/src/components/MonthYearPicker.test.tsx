...
describe('MonthYearPicker', () => {
  it('renders 12 cells with Spanish mesAbreviado for the displayed year', () => {
    ...
    expect(
      screen.getByRole('button', { name: /enero 2026/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('button', { name: /diciembre 2026/i }),
    ).toBeInTheDocument();
  ...
  it('marks only the cell matching periodo as active', () => {
    ...
    expect(screen.getByRole('button', { name: /junio 2026/i })).toHaveAttribute(
      'aria-pressed',
      'false',
    );
  });
  ...
  it('disables months after the current month when the displayed year equals the current year', () => {
    ...
    expect(
      screen.getByRole('button', { name: /julio 2026/i }),
    ).not.toBeDisabled();
  });
  ...
  it('prev-year button is always enabled and shows the previous year grid', async () => {
    ...
    expect(
      screen.getByRole('button', { name: 'Año anterior' }),
    ).not.toBeDisabled();
    await user.click(screen.getByRole('button', { name: 'Año anterior' }));

    expect(
      screen.getByRole('button', { name: /julio 2025/i }),
    ).toBeInTheDocument();
    // 2025 is fully past, so no month should be disabled anymore.
    expect(
      screen.getByRole('button', { name: /diciembre 2025/i }),
    ).not.toBeDisabled();
  });
  ...
  it('next-year button is disabled at the current year and enabled after navigating back', async () => {
    ...
    expect(
      screen.getByRole('button', { name: 'Año siguiente' }),
    ).toBeDisabled();
