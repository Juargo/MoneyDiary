...
describe('esMesActual', () => {
  it('returns true when periodo matches the current UTC month', () => {
    expect(esMesActual('2026-07', new Date('2026-07-19T12:00:00.000Z'))).toBe(
      true,
    );
  });

  it('returns false when periodo is a past month', () => {
    expect(esMesActual('2026-06', new Date('2026-07-19T12:00:00.000Z'))).toBe(
      false,
    );
  });

  it('returns false when periodo is a future month', () => {
    expect(esMesActual('2026-08', new Date('2026-07-19T12:00:00.000Z'))).toBe(
      false,
    );
  });
});
...
describe('esPeriodoFuturo', () => {
  it('returns true for a period after the current one', () => {
    expect(
      esPeriodoFuturo('2026-08', new Date('2026-07-19T12:00:00.000Z')),
    ).toBe(true);
  });

  it('returns false for a period before the current one', () => {
    expect(
      esPeriodoFuturo('2026-06', new Date('2026-07-19T12:00:00.000Z')),
    ).toBe(false);
  });

  it('returns false when periodo equals the current month (edge case)', () => {
    expect(
      esPeriodoFuturo('2026-07', new Date('2026-07-19T12:00:00.000Z')),
    ).toBe(false);
  });
});
