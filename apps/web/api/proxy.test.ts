...
function fakeRequest(
  target: URL,
  options: { method: string; headers: Record<string, string> },
  cb: (res: Readable & { statusCode: number; headers: Record<string, string | string[]> }) => void,
) {
...
vi.mock('node:https', async (importOriginal) => {
  ...
  const request = (...args: unknown[]) => fakeRequest(...(args as Parameters<typeof fakeRequest>))
...
function createRes(): FakeRes {
  ...
  res.setHeader = vi.fn((key: string, value: string | number | readonly string[]) => {
...
describe('proxy handler', () => {
  ...
  it('preserves the original query string alongside the reconstructed path', async () => {
    const res = createRes()
    await handler(createReq({ url: '/api/proxy?upstream=resumen&periodo=2026-07' }), res)

    expect(`${captured?.target.pathname ?? ''}${captured?.target.search ?? ''}`).toBe('/api/resumen?periodo=2026-07')
  })

  it('strips any client-supplied x-api-key header and replaces it with the server-side key', async () => {
    const res = createRes()
    await handler(createReq({ headers: { 'x-api-key': 'attacker-supplied' } }), res)
  ...
  it("relays the browser's Sec-Fetch metadata as x-fwd-sec-fetch-*", async () => {
    const res = createRes()
    await handler(
      createReq({ url: proxyUrl('auth/demo'), headers: { 'sec-fetch-dest': 'document', 'sec-fetch-mode': 'navigate' } }),
      res,
    )
  ...
  it('sets x-fwd-sec-fetch-* ONLY from the real request, dropping a client-forged one (unforgeable, like x-api-key)', async () => {
    const res = createRes()
    await handler(
      createReq({ url: proxyUrl('auth/demo'), headers: { 'sec-fetch-dest': 'image', 'x-fwd-sec-fetch-dest': 'document' } }),
      res,
    )
  ...
  it('does NOT follow a backend redirect — forwards the 302 + Location + Set-Cookie so the browser follows it WITH the session cookie', async () => {
    upstream = {
      ...
      headers: {
        location: '/',
        'set-cookie': 'md_session=abc123; HttpOnly; Path=/; SameSite=Strict; Secure',
    ...
    expect(res._headers['set-cookie']).toBe('md_session=abc123; HttpOnly; Path=/; SameSite=Strict; Secure')
  ...
  it('forwards the upstream Set-Cookie response header back to the browser (login sets md_session)', async () => {
    ...
    expect(res._headers['set-cookie']).toBe('md_session=abc123; HttpOnly; SameSite=Strict; Path=/; Max-Age=604800')
  })
  ...
  it('forwards a multipart/form-data POST body byte-for-byte with the boundary content-type intact', async () => {
    ...
    const bodyChunks = [multipartBody.subarray(0, 10), multipartBody.subarray(10, 25), multipartBody.subarray(25)]
    ...
    expect(Buffer.compare(Buffer.concat(captured?.body ?? []), multipartBody)).toBe(0)
    expect(captured?.options.headers['content-type']).toBe(`multipart/form-data; boundary=${boundary}`)
  })
})
