 ...
 * `meses` payload (`GET /api/resumen/anual`) — a presentation-level
 * percentage over two backend-served totals, never parseFloat: the delta is
 * exact BigInt arithmetic on the raw decimal strings, rounded half away
 * from zero to an integer percent.
 *
 * Deliberate mirror of `apps/web/src/domain/variacion-ingreso.ts`
 * (ADR-008/024: no shared package) — keep both in sync, same discipline as
 * the `veredicto-semaforo` pair. Theme-free and fetch-free: pure function
 * over a structural input.
 *
 * Null (no pill, the honest degradation) whenever the comparison has no
 * base: current periodo missing from the window, current periodo is the
 * window's first month, either month empty (`sinIngreso`), previous total
 * zero, or a malformed amount.
 */
