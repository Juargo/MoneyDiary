/**
 * Pure SVG pie geometry — no react-native-svg import here so the arc math is
 * unit-testable in plain Jest. Angles are degrees measured CLOCKWISE from the
 * top (12 o'clock), matching the mockup's orientation.
 */
 ...
 *
 * `rInterior` (US-050, port of web's `apps/web/src/domain/pie-geometry.ts`)
 * is a TRAILING OPTIONAL parameter — the donut hole radius, in absolute px,
 * `0` by default. `rInterior === 0` returns the exact same string this
 * function always returned (byte-for-byte regression contract,
 * `pie-geometry.spec.ts`). `rInterior > 0` draws an annular wedge instead:
 * outer arc forward, a straight line inward, then the inner arc BACKWARD
 * (inverted sweep flag) before closing — the wedge starts at the outer
 * boundary, never at `M cx cy`. The full-360° branch mirrors this with two
 * independent subpaths (outer circle forward, inner circle with the opposite
 * sweep flag) so the default non-zero fill rule punches the hole — without
 * it, a single bucket holding 100% of spend would render a filled disc
 * instead of a ring. `rInterior` is clamped to `[0, r)`: a value at or above
 * `r` degrades to the filled wedge instead of emitting an inverted,
 * self-crossing path — degrade, never throw (same discipline as
 * `montoSeguro`).
 */
