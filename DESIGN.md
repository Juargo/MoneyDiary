...
- **Warning** (`warning` surface + `warning-border` + `warning-foreground` + `warning-accent` hover wash): the amber notice family for the app-shell `DemoBanner`, the "sin categoría" callout, and inline validation hints. Unifies what used to be a mix of amber-800/900 text into one `warning-foreground`. The upload flow's `DemoUploadNudge` is deliberately NOT on this family (2026-08-30 minimalist pass): it is an invitation, not a caution, so it sits on the neutral `bg-muted/40` surface. The same neutral surface (`bg-muted/40` + Mist border, `rounded-lg`) also holds the upload preview's cartola identity block in `PreviewMuestra` (2026-08-30 polish): banco + three number-over-label counts + the "nada se ha guardado" footnote sit on it so the file's metadata reads as one object, visibly distinct from the "Movimientos" review list below it. That list is a full-bleed `<section>` (`-mx-4` against the preview card's `p-4`): a sticky header band (`border-y`, opaque `bg-muted` — it sticks over the rows, so the translucent `/40` wash is not an option here; the per-date accordion headers below use the `/40` wash as a lighter step) holding the `h3`, filter, select-all and progress; inset white rows grouped by date, each date an accordion (`h4` > `button[aria-expanded][aria-controls]`, heading text "{fecha} · N movimientos", `ChevronDown` rotated -90° when closed, collapsed lists `hidden` not unmounted; all open by default); and a closing `border-b` so the action buttons below sit under a visible edge. The upload status line (`role="status"`) leads with a state glyph (spinner / `CircleCheck` in `semaforo-verde-foreground` / `CircleAlert` destructive) and steps up to `font-medium text-foreground` once a step lands.
...
- **Cargo** (`cargo-foreground`, 2026-08-30): expense-amount text in the upload preview rows (`FilaRevision`), paired with `ingreso-foreground` for abono on the same row. Same literal as `semaforo-rojo-foreground` (#c70036, AA) but its own token; not `destructive`, which means error. Rule (2026-08-31): a row shows only the amount that moved, signed by direction (`-$6.990` cargo, `+$596` abono, both when both are non-zero, cargo first); a row with nothing on either side shows one neutral `$0`. The `Cargo`/`Abono` labels stay in the DOM as `sr-only` `<dt>`s so sign + color are never the only carriers. The description is always complete (wraps, never truncated).
...
Single-choice among the 50/30/20 buckets rendered as native radio inputs (`sr-only`) with label chips inside a `fieldset[aria-label]`: square corners, 32px min-height, 14px text. Resting chip = `bg-card` + Mist border, hover `accent`; the checked chip takes its bucket's pastel FILL (`necesidades`/`gustos`/`ahorro`, `muted` for "Sin categoría") and GAINS definition rather than losing it — an ink `border-foreground` outline, `shadow-sm` elevation and semibold text (the pastels are light by the Two-Tier Color Rule, and the original `border-transparent` checked chip read as the weakest chip in the group). Geometry never changes on selection: the chips sit in a fixed grid, so scaling the checked one would shove its neighbours on every click. Colour is never the only carrier — outline, elevation and weight all carry it. Focus uses the shared 3px `ring/50` grammar on the label via `peer-focus-visible`. Chips sit in an equal-width 2×2 grid at every breakpoint (a one-row variant clipped labels inside the half-row column), content centered, no `truncate`, `min-h-8` so a long label wraps rather than clips. Each chip leads with a 14px lucide glyph from `lib/bucket-icons.ts` (`House` Necesidades, `Sparkles` Gustos, `PiggyBank` Ahorro, `CircleDashed` Sin categoría), `aria-hidden` so radio names stay the labels. Used by the upload preview rows — **except income rows**: a row the backend classified as `Ingreso` renders NO control at all (not even a disabled one — a disabled control still reads as a choice you could have made), just a bold `ingreso-foreground` green "Ingreso" word in the row header (plain text, NOT a badge — "Duplicado" is an exception worth a chip, "Ingreso" is simply what the row is, and the green ties it to the abono figure on the right) and a quiet `text-xs` line saying it is classified automatically. `CommitIngestaUseCase` Rule 2 already persists `{ Ingreso, null }` for those rows and discards any overlay, so any control there promised an edit the server would throw away; they are also excluded from bulk selection and count as classified in the progress readout (`domain/clasificacion-preview.ts`). Full opacity, unlike duplicates — the transaction IS being imported, it simply needs no decision. The categoría `<select>` beneath the control only renders once a bucket other than "Sin categoría" is checked, and settles in with `categoria-in` (200ms fade + 4px drop, keyed by bucket so a switch replays it): a state-transition cue, deliberately not a fake loading state, since the catalog is already in memory.
