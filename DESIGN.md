...
- **Cargo** (`cargo-foreground`, 2026-08-30): expense-amount text in the upload preview rows (`FilaRevision`), paired with `ingreso-foreground` for abono on the same row. Same literal as `semaforo-rojo-foreground` (#c70036, AA) but its own token; not `destructive`, which means error. Rule: a `$0` in either column stays neutral `text-foreground` — color marks money that moved, never an empty cell.
