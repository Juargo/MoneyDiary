...
`Categoria` MUST be unique per `(userId, bucketId, nombre)` — this clause is
superseded by ADR-042/CAT038-13, which supersedes ONLY the prior
`(userId, nombre)` uniqueness scope of this requirement; every other
guarantee here (NOT NULL `userId`, bootstrap-user ownership, no FK
repointed by the migration) is unchanged. The previous global `(userId,
