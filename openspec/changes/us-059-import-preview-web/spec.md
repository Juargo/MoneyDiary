# Spec: US-059 — Web: Upload Cartola + Editable Review Preview

**Change**: `us-059-import-preview-web`
**Type**: New capability (frontend-only; consumes existing US-057 backend contract)
**Depends on**: `openspec/specs/ingesta-preview-commit/spec.md` (canonical backend contract, already live)

---

## Purpose

Replace the web's legacy one-shot upload flow with the US-057 preview → review → commit
two-phase interaction. The web client becomes the first consumer of the canonical
`POST /api/ingestas/preview` + `POST /api/ingestas/commit` endpoints.

This is a **presentation-only** change (ADR-024): all validation, dedup detection,
...
| Layer | Focus |
|-------|-------|
| Unit — `esPreviewIngestaDto` guard | Rejects legacy-only shape; accepts canonical shape; rejects null `filas`/`resumen` |
| Unit — `FilaRevision` | Duplicate badge + disabled selects when `esDuplicado=true`; enabled selects when false; accessible label presence |
| Unit — `PreviewMuestra` | Resumen header values; "nothing saved yet" text; all rows rendered (no truncation at 50) |
| Unit — `SubirCartola` state machine | `idle→previsualizando→(preview-error \| preview-listo)→committing→(error \| exito)` transitions; edits reset on discard and on new-file pick; edits PRESERVED on commit error (D-11); trivially empty at preview-error (preview never succeeded); navigate `/` on commit success |
| Unit — `useCommitIngesta` | Mutation sends multipart with `file`+`edits`; `onSuccess` invalidates correct query keys (navigation is asserted in the `SubirCartola` tests — it lives at the call site, design D-05) |
| Unit — cascade | Bucket selection filters categoría options to that bucket only; changing bucket resets categoría |
| Integration — commit flow | Full upload→preview→edit→commit cycle with mocked API returns correct FormData structure |
| Regression | `useIngesta`/`postIngesta` exports still present and unchanged; the hardened `esPreviewIngestaDto` guard IGNORES legacy `muestra`/`estructura` entirely; it returns false for responses lacking canonical `filas`/`resumen` (regardless of whether legacy fields are present) |
