...
- [x] **C1.5.** Write failing supertest specs (fake-container pattern, design §11.2) asserting: `GET /api/auth/google` and `GET /api/auth/google/callback` return **404** (not 401, not 500) when `container.googleAuth` is `undefined` — this is the regression test for the §4.4 routing trap (unmounted route falling through to `sessionMiddleware` → 401).
- [x] **C1.6.** Implement `registrarAuthGoogleDeshabilitado(router)` (the disabled-stub route registrar, ~8 lines per design §4.4) in `apps/api/src/infrastructure/http-express/routes/auth-google.routes.ts` (stub half only — the real handlers land in C2). Run C1.5 green. **Implementation note (apply-time deviation, documented not improvised):** `app.ts` mounts `registrarAuthGoogleDeshabilitado` UNCONDITIONALLY in C1 (not the `container.googleAuth !== undefined ? registrarAuthGoogle(...) : ...` branch) — `registrarAuthGoogle` does not exist until C2, and building a throwaway placeholder to pre-wire a branch whose real arm has no agreed signature yet would be speculative (YAGNI). AUTH-16's activation truth is still fully observable in C1 via `GET /api/auth/capabilities`, which reads `container.googleAuth` directly. C2 replaces the unconditional call with the real branch — no route-mounting code from C1 needs to be re-tested, only extended.
- [x] **C1.7.** Write failing unit test + supertest spec for `GET /api/auth/capabilities`: 401 without `x-api-key` (AC-10); 200 `{ "googleLoginEnabled": true }` when `container.googleAuth` is defined; 200 `{ "googleLoginEnabled": false }` when undefined; session-public (reachable with no session cookie/Bearer, per AC-10).
- [x] **C1.8.** Implement `apps/api/src/infrastructure/http-express/routes/auth-capabilities.routes.ts` (always mounted, unconditional per design §4.5) + `apps/api/src/infrastructure/http-express/schemas/auth-capabilities.schema.ts` (Zod schema for the response body, following the existing per-schema pattern). Run C1.7 green.

### Infrastructure — log-redaction hardening (design §6.3 a+b)

- [x] **C1.9.** Write failing unit test for `apps/api/src/infrastructure/http-express/middleware/redactar-query-params-sensibles.spec.ts` (or co-located with `request-logger.middleware.ts`): `code`/`state`/`id_token`/`access_token`/`refresh_token`/`token`/`code_verifier` values redacted to `[REDACTED]`; `periodo`/`anio` and other non-sensitive params untouched; malformed/no-query URLs handled safely.
- [x] **C1.10.** Implement `redactarQueryParamsSensibles(url: string): string` (pure function, ~20 lines) and wire it as `serializers.req` in `createRequestLoggerMiddleware` (`apps/api/src/infrastructure/http-express/middleware/request-logger.middleware.ts`). Run C1.9 green. **Apply-time discovery beyond the literal design text:** `pino-http`'s default `req` serializer emits TWO independent fields carrying the query string — `url` (the raw string, what design §6.3(a) names) AND a separate already-parsed `query` object (Express's `req.query`, verified empirically against the real emitted log shape). Redacting only `url` would still leak `code`/`state` via `query`. Added a second pure function `redactarQueryObjectSensible` (same file) and wired both into a custom `serializers.req` that wraps `pino-http`'s own default (the custom serializer receives the ALREADY-serialized request, not the raw `IncomingMessage` — confirmed in `pino-std-serializers`' `wrapRequestSerializer` source). Regression test added to `request-logger.middleware.spec.ts` (mirroring the Set-Cookie test) proves neither vector leaks for a callback-shaped URL.
- [x] **C1.11.** Investigate P5 (whether `res.headers` including `Set-Cookie` is currently logged). Write a failing regression test capturing the NDJSON pino stream for a login/demo response and asserting no token value appears. Add `'res.headers["set-cookie"]'` to `SENSITIVE_REDACT_PATHS` (`apps/api/src/infrastructure/logging/pino-logger.ts`) if the test proves the leak exists. Run the regression test green. **Apply-time finding: already done, independently.** P5 was confirmed and fixed in a separate, already-merged PR (`fix/redact-set-cookie-logs`, PR #239, merged to `main` before this slice branched) — `SENSITIVE_REDACT_PATHS` already includes `'res.headers["set-cookie"]'` and `request-logger.middleware.spec.ts` already carries the regression test asserting the token value never appears. Verified present on this branch (inherited from `main`); no new work needed here. This satisfies the P5 recap note ("investigated and closed for the Google path inside C1 regardless of outcome").
- [x] **C1.12.** Write a failing test asserting `errorMiddleware` (`apps/api/src/infrastructure/http-express/middleware/error.middleware.ts`) never serializes `LoginConGoogleFallidoError`'s `motivo` property into any client-facing response body — pass a `LoginConGoogleFallidoError` with each `motivo` value through the middleware and assert the JSON response contains only the generic message, never a `motivo` key or its value. This is an AUTH-15 consistency guard shared with C2 (re-run against the real callback route's error path in C2.3/C2.7, not just this unit-level check) — 4R carry-forward. **Apply-time note:** `error.middleware.ts` already structurally guarantees this (fixed generic message, no error-property serialization) — the new `error.middleware.spec.ts` is a characterization/regression GREEN-from-first-run test, not a RED→GREEN bugfix cycle. Kept as a permanent guard so a future edit to `errorMiddleware` cannot silently reintroduce leakage.
...
- [x] **C2.4.** Implement the real handlers in `apps/api/src/infrastructure/http-express/routes/auth-google.routes.ts` (`registrarAuthGoogle`), replacing the C1 placeholder wiring so the `app.ts` branch now calls the real registrar when `container.googleAuth` is defined:
  ...
  - Run C2.3 green.
  - **Apply-time discovery beyond the literal design text:** `GoogleAuthGraph` (from C1.4) only exposed `iniciador` — nothing in C1 consumed `.verificar()` yet. The callback needs the verify role separately (design §5.1: the use case never verifies, the route does, before calling it). Extended `GoogleAuthGraph` with a `verificador` field — the SAME `OpenIdClientGoogleAdapter` instance as `iniciador`, narrower-typed (ISP, design §4.1), not a second instance. `crear-auth-google.spec.ts` extended accordingly (additive, existing assertions untouched).
  - **Infra-fault decision, resolved (no design §6 carve-out was written, so the default per this task's own instruction applies):** any unmodeled throw from `verificador`/`loginConGoogle`/their collaborators in the callback (and from `iniciador` in initiate) is caught and produces the uniform `302 /login?error=google` — never a 500. Log level is the distinguishing observability signal: `Result.fail` branches (modeled auth failures) log via `appLogger.warn(..., { motivo })`; unmodeled exceptions (including the benign §5.1 partial-link state, which always surfaces as a throw since `emitirSesion` doesn't catch `sessions.crear`) log via `appLogger.error(..., { errorName })`, no `motivo` (none available). No rollback attempted — self-repairs via `buscarPorGoogleSub` on retry, per design.
  - `urlCallback` passed to `verificador.verificar()` is rebuilt from the CONFIGURED `redirectUri` (deps, `env.GOOGLE_REDIRECT_URI`) plus only the incoming query string — never from request headers (design §7 open-redirect guard). Covered by a dedicated test using a spoofed `Host`/`X-Forwarded-Host`.

### Composition — wire the shared rate limiter instance

- [x] **C2.5.** Update `crearAuthGoogle` (from C1.4) to construct the `IpRateLimiter` with prefix `google:ip:`, budget 10/15min (hardcoded constants, no new env vars, per design §6.4). Confirm `crearAuth`'s existing `demo:ip:` limiter is unaffected (budget/behavior unchanged, only the class name changed in C1.2). — **Verified, no new code**: this was already done in C1.4/crear-auth-google.ts. Re-confirmed `demo:ip:` prefix/budget untouched in `crear-auth.ts` (grep + full test suite green).

### OpenAPI

- [x] **C2.6.** Register `/api/auth/google` `get` and `/api/auth/google/callback` `get` in `openapi-document.ts` (appended at the end of `paths`, design §10): `302`(+`Location`)/`403`/`404`/`429` for initiate; `302`(both outcomes, documenting the AUTH-15 indistinguishability as the contract itself)/`403`/`404`/`429` for callback. Run `pnpm api openapi:emit` and `openapi:check`.

### Capabilities/route drift guard (4R C1 carry-forward)

- [x] **C2.6a.** Boot-time (or CI) guard that FAILS if `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` are present but the real Google routes aren't registered — closes the C1-only drift where `/api/auth/capabilities` reports `true` while `/api/auth/google` 404s. (R4 C1 carry-forward; can only exist once C2 provides the real routes.) — **Implemented as `assertGoogleAuthActivationConsistency(env, container.googleAuth)`** (`apps/api/src/composition/assert-google-auth-activation-consistency.ts`), called from `server.ts` right after `createContainer`, before `app.listen()`. Throws (fails boot) if both credentials are present but `container.googleAuth` is `undefined`. Note: since C2's `app.ts` now ties route mounting to the SAME `container.googleAuth !== undefined` condition capabilities reads, the original C1-era routing-level drift can no longer occur by construction — this guard closes the remaining possible cause, a composition bug where `crearAuthGoogle` silently returns `undefined` despite valid credentials.

### Slice close-out

- [x] **C2.7.** `pnpm api test` and `pnpm api test:integration` green (integration: one end-to-end callback with a verifier **double**, never live Google, asserting a real `Session` row with SHA-256 hash + `expiresAt` = creation + 7 days, indistinguishable from a password-login row — design §11.3). `pnpm api exec tsc --noEmit` green. — **Evidence:** unit 1308/1308 (163 files), integration 60/60 (14 files, includes the new `auth-google-callback.int-spec.ts`), tsc clean, `pnpm audit --audit-level=high` exit 0 (no new high vulns — no dependency changes in this slice). `pnpm api test:e2e` was also attempted for extra confidence; 3 pre-existing files unrelated to Google (`app`/`auth-login`/`auth-rate-limit`) passed, 7 files failed on a pre-existing environment gap (`SEED_USER_EMAIL`/`SEED_USER_PASSWORD` + `pnpm api test:db:setup` not run in this session) — zero failures touch Google/auth-google, not a regression from this slice.
- [ ] **C2.8. — MANUAL GATE, BLOCKS MERGE. NOT executed by this apply batch — requires a human with Google Cloud Console access and real browsers.** Run the full design §11.4 checklist against a real Google Cloud Console test client (requires P1/P2 done and Google credentials set in the target environment) and paste the result into the PR description before merging:
...
- [ ] **C2.9.** Open PR #4, dependency diagram 📍 on this PR, prior dependency = PR #3. PR description must include the pasted §11.4 checklist results (C2.8) before requesting merge. — **Deferred**: apply phase stops after the last commit per orchestrator instructions (no push, no PR); PR creation happens after the 4R review gate and, per C2.8, cannot request merge until the manual checklist is pasted in.
...
- [x] **D1.** Write failing component test for `apps/web/src/api/capabilities.ts` (TanStack Query hook, mirrors existing `apps/web/src/api/*` hand-written-DTO pattern): fetches `/api/auth/capabilities`, returns `{ googleLoginEnabled: boolean }`.
- [x] **D2.** Implement `apps/web/src/api/capabilities.ts`. Run D1 green.
...
- [x] **D4.** Implement `apps/web/src/components/GoogleLoginButton.tsx` (or repo-conventional path) — top-level `<a href>`, same pattern as the landing's demo link. Wire into `/login` below the existing form. Run D3 green.
- [x] **D5.** Write failing test for `apps/web/src/routes/login.tsx` `validateSearch` extension: whitelists `error: 'google'` only (any other `?error=` value is stripped, matching `sanitizeRedirect`'s discipline — never echo attacker-controlled values); existing `redirect` behavior unchanged. **Apply-time discovery beyond the literal design text:** the real `@tanstack/react-router` match pipeline exposes TWO parallel search representations per match — a "loose" accumulated one (`match.search`, used to build `router.state.location.search`/the URL bar) that inherits ancestor-contributed raw keys via `Object.assign` and does NOT delete a key a child's `validateSearch` chooses to omit, and a "strict" one (`match._strictSearch`, what `Route.useSearch()` actually returns to components) that is fully owned by each route's own `validateSearch` output. A first draft of this test asserted on `router.state.location.search`, which — like the pre-existing `?redirect=` case — retains an attacker-controlled raw value in that loose/URL-bar representation even though `validateSearch` correctly strips it. This is **not exploitable** (nothing in the app reads `location.search` directly; `LoginPage`/`LoginForm` only ever consume `Route.useSearch()`, i.e. the strict, validated object), so no app-code change was needed — but it means the test had to assert against `Route.options.validateSearch` directly (the actual security boundary) plus the rendered-DOM absence of the payload, not against `router.state.location.search` equality, which would have been asserting the wrong surface. Flagged for `sdd-verify`/R1: the pre-existing `?redirect=` sanitization has the same loose-search characteristic (cosmetic-only, not app-observable) and was not previously covered by a router-level regression test — worth a follow-up test, out of this slice's scope.
- [x] **D6.** Implement the `validateSearch` extension per design §6.2. Run D5 green.
- [x] **D7.** Write failing test asserting `/login` renders the existing generic `role="alert"` component when `?error=google` is present in the resolved search params (reuse `LoginForm`'s existing alert style — do not introduce a second alert component).
- [x] **D8.** Implement the `?error=` rendering wiring in `login.tsx`/`LoginForm`. Run D7 green. **Implementation note:** the alert and `GoogleLoginButton` are rendered by `LoginPage` (the route container) below `LoginForm`, not inside `LoginForm` itself — `LoginForm`'s own internal state/behavior (and its 15 existing tests) are untouched, per this slice's explicit boundary.
- [x] **D9.** `pnpm web test` green (61 files, 560 tests). `pnpm web typecheck` green (`tsr generate && tsc -b`). `pnpm web build` also run clean as an extra check.
- [ ] **D10.** Manual smoke: with Google configured locally, confirm the button appears and completes end-to-end (this overlaps with C2.8 step 1 but is now exercised through the actual UI element, not a raw URL). — **Not executed by this apply batch**: requires Google credentials configured locally (P1/P2) and a human driving a real browser: same category as C2.8, deferred to the manual gate.
- [ ] **D11.** Open PR #5, dependency diagram 📍 on this PR, prior dependency = PR #4. This is the **last** PR in the web chain. — **Deferred**: apply phase stops after the last commit per orchestrator instructions (no push, no PR); PR creation happens after the 4R review gate.
