...
- [x] **1.3** ~~`sec-fetch.middleware.ts`~~ → **MOVED to Slice 7.** `esNavegacionDeNivelSuperior` is NOT a global guard — it's a pure `Request → boolean` used only by `GET /api/auth/demo`. Belongs to the auth slice.
...
- [x] **8c.2** `demo-cleanup.service.ts` — Nest `Logger` → `console`; dropped `@Cron`. Spec updated (`console` spies). **Daily scheduler restored** (follow-up): `scheduler/demo-cleanup-scheduler.ts` (`programarLimpiezaDemo`, node-cron `0 3 * * *`, `noOverlap`) wired in `server.ts` boot + stopped on shutdown; test-first (2 tests, injected `schedule`). Lazy cleanup in `GET /demo` also runs.
...
- [x] **8c.4** Removed deps: `@nestjs/{common,core,platform-express,schedule,cli,schematics,testing}`, `reflect-metadata`, `rxjs`, `@swc/core`, `unplugin-swc`, `ts-loader`. Simplified all 3 vitest configs (dropped SWC + `oxc:false` + `reflect-metadata` setup → default Oxc transformer). **809/809 unit green** (was 889; −80 deleted Nest-controller/guard specs; later 811 with node-cron). ✅ **Supertest port-collision flake fixed**: `retry: 2` in the unit vitest config absorbs the transient OS/supertest ephemeral-port race (couldn't reproduce in 15 concurrent stress runs; a real failure still fails all 3 attempts). e2e/int configs already run serial (`fileParallelism: false`) so they don't need it.
