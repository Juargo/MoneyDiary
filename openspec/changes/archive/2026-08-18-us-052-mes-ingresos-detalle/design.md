...
| `app.ingresos-mes.spec.ts` | NEW | **6** | 401 sin `x-api-key` (1); 401 api-key sin sesión (1); 200 con api-key + sesión y el `userId` DE LA SESIÓN fluye al use case (MID-06) (1); 400 scrubbed en `PeriodoInvalidoError` — input crudo ausente del body + mensaje de dominio (1); body 200 real cumple `ingresosMesResponseSchema` — exactamente `{total, conteo, transacciones}` (MID-03: sin meta/porcentaje/estado, verificado DENTRO de este caso) + array wire completo de las 3 txs con `origen` banco (1); rama Manual del use case real via fake reader — `banco: ''` → `'Manual'` (MID-02 dead-code proof) (1) |
