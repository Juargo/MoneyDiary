...
### PR7 post-validation corrections (orchestrator)

- **The rollback-coupling claim in the PR7 work-unit evidence is WRONG and is superseded here.** `GrupoMovimientosMobile.tsx:113` reads `icono` off its `grupo` prop, which `BucketDetalleScreen.tsx:263` fills with `estado.dto.grupos[idx]!` — the wire DTO (it has carried `icono?: string | null` since PR3a), NOT `GrupoDetalleMesViewModel`. The two PR7 commits are therefore independently revertible. Verified by the orchestrator: `viewModel.grupos` is used at `BucketDetalleScreen.tsx:200` (emptiness) and `:260` (iteration) only.
- **Known divergence from MDET-03 (data path, not behaviour).** MDET-03 says each group is rendered "from `aDetalleBucketMesViewModel`" and shows "the group's `icono`". The screen iterates the view model but passes the DTO group as the prop, so the badge reads the DTO field. Observable behaviour matches the requirement exactly — both objects carry the same value, normalized the same way — but the source is not the one the spec names. `GrupoDetalleMesViewModel.icono` (task 7.2) consequently has no production reader today.
- **Decision (orchestrator):** keep the view-model field and do NOT rewire the prop in this slice. The prop source is pre-existing mobile structure, untouched by this change; switching it is a typed refactor of a component whose props are the DTO shape, which belongs in its own change rather than in the last slice of this one. Keeping the field preserves parity with web (where `BucketDetalleMesPage` does feed `viewModel.grupos`) and makes that future rewiring a one-line change.
- **Follow-up owed:** align mobile's detalle prop source with the view model (or amend MDET-03 to describe the DTO path), tracked for after this change archives.

## PR8 — Icon label collision fix (orchestrator, maintainer-approved)

- **Problem:** `ETIQUETA_ICONO['piggy-bank'] = 'Ahorro'` was the exact accessible name of the `Ahorro` bucket option. Since PR6b both controls share the Configuración screen, so two radios carried identical accessible names. Each radiogroup has its own container label, which disambiguates sequential screen-reader navigation but NOT a flattened control list (VoiceOver rotor, Voice Control "tap Ahorro"). Same ambiguity in both clients.
- **Fix:** renamed to 'Alcancía' in `apps/web/src/lib/iconos-categoria.ts` and `apps/mobile/src/components/iconos-categoria.ts`, each with a comment recording why. Label-only: the mirror spec pins icon IDs, not labels, so no contract change; no test pinned the literal string (consumers read it through the constant).
- **Verification:** web iconos-categoria + SelectorIcono specs 13/13 and `typecheck` clean; mobile iconos-categoria, SelectorIcono, NuevaCategoriaForm, EditarCategoria and CategoriasPanel specs 73/73 and `tsc` clean.
- The `within(...)` query scoping PR6b added stays: it is more precise regardless, and reverting it would be churn.
