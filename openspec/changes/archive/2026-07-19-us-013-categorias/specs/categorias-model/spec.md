...
existing `Transaccion`: matched rows get `categoriaId` + reconciled
`bucketId`; unmatched rows get `categoriaId=null` and keep/receive
...
`ALLOW_DESTRUCTIVE_DB` gate (reject production connection strings) and MUST
be idempotent (safely re-runnable, same result on repeat).
