...
| `MiniDistribucionPie.tsx` | NEW | port of MOBILE's own `DistribucionPie` — a 44px DONUT (deliberate divergence from web's solid mini; family consistency with the main ring — see tasks.md PR5a deviation 5): fixed `size = 44`, label-less, ≤4 `<Path>`, muted placeholder circle when `tajadas` is empty. Carries **no** accessibility props — the parent `Pressable` is `accessible`, which collapses the subtree into a single AT node |
| `states/Loading.tsx` | unchanged (judgment revert) | `ResumenAnual` does NOT reuse this component — it renders its own inline loading view (same `flex-1`-collapse rationale as `ErrorAnual`'s own local error view: the shared `states/*` components need a flexed full-screen ancestor to center correctly, which a section nested inside a `ScrollView` is not). T5b.1's `mensaje` prop had zero production consumers on that premise and was reverted by judgment-day review (PR5b) |
