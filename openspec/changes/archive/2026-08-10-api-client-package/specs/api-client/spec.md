# API Client Package Specification (New Capability)
...
B in `explore.md`) — no runtime client, no `TokenStorage`, no `errors.ts` ship here (see Non-Goals).
...
| Excluded | Reason |
|----------|--------|
| `client.ts` (openapi-fetch, interceptors) | Deferred; a future slice once workspace/CI/Metro mechanics are proven (see proposal) |
...
| `tsup` / any build step | Explicitly out — a types-only package needs none (see proposal "Approach") |
| npm publication / independent release-please entry | Internal `workspace:*` only |
| Runtime validation of API responses | Generated types are compile-time only; runtime money-safety guards stay in each app (see `web-app` and `mobile-app` deltas) |
