# Spec: US-005 Detección de datos duplicados (superseded copy)

> **This flat file is a historical artifact, kept only for the phase-history trail.**
> The canonical spec for this change lives at
> `specs/ingesta-duplicate-detection/spec.md` in this same change folder, and —
> after archive — is promoted to `openspec/specs/ingesta-duplicate-detection/spec.md`
> as the go-forward source of truth.
>
> During archive reconciliation (2026-07-21), the canonical (nested) spec's
> "Response shape reflects counts" scenario was corrected to match the shipped
> 2-field response (`totalTransacciones` + `duplicadosOmitidos`, no
> `transaccionesImportadas`). This flat copy is intentionally left unmodified
> below as the as-drafted historical record — do NOT read it as current
> behavior; read `specs/ingesta-duplicate-detection/spec.md` (this folder) or
> `openspec/specs/ingesta-duplicate-detection/spec.md` (canonical, post-archive)
> instead.
...
> **As-drafted note (see banner above):** the shipped implementation does NOT
> add `transaccionesImportadas` — it is redundant with `totalTransacciones`,
> which already means "imported count" (unchanged from pre-existing code).
> The canonical spec reflects the shipped 2-field shape.
