...
- [ ] 3a.1 RED: `apps/api/src/infrastructure/http-express/schemas/categorias.schema.spec.ts` — `icono` optional-nullable on create/update AND on the response (D-11: `.nullable().optional()`, generated type stays widen-only); `.strict` rejects unknown keys
