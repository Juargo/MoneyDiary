...
<!-- REAL NUMBERS (applied 2026-08-19): CategoriaFila.tsx 45L + CategoriaFila.spec.tsx 95L + CategoriasPanel.tsx 91L + CategoriasPanel.spec.tsx ~165L + configuracion.tsx +2 (import CategoriasPanel) - 4 (replaced placeholder) + configuracion.spec.tsx +3 changes (~394 total ledger lines). 19 new tests (6 CategoriaFila + 13 CategoriasPanel, incl. the Otros case from the JD fix round). Full suite: 48 suites / 554 tests (535 baseline + 19 new). tsc --noEmit clean. RED evidence: T5b.1 → Cannot find module './CategoriaFila'; T5b.3 → Cannot find module './CategoriasPanel'. Key discoveries: PatronDto requires categoriaId in fixtures (tsc-enforced); RNTL render must be awaited or screen queries fail with "render function has not been called"; D-13 hint "Toca una categoría para editarla o eliminarla." used as mobile variant; ETIQUETA_BUCKET from theme/colors.ts maps Deseos→Gustos correctly.

Deviations (JD fix round 2026-08-19):
- ORDER test: originally assembled heading nodes individually via getByTestId — tautological (passes regardless of DOM order). Fixed to use a single screen.getAllByRole('header') query; CategoriasPanel sets accessibilityRole="header" + accessible={true} on each group heading Text node. The claim in the original note ("getAllByRole('heading') requires accessible={true} — used testID-based assertion instead") was a factual error: accessible={true} is present and getAllByRole('header') works correctly (falsifiability confirmed: reversed array produces a test failure). 13 total tests after adding the Otros case.
- Empty state: states/Empty was NOT reused for the empty state. states/Empty hardcodes income copy ("Sin ingresos registrados este período"); reusing it for the category catalog would embed incorrect user-facing text. The panel implements its empty state inline with category-specific copy ("Todavía no tienes categorías" / "Crea tu primera categoría…") — deliberate. -->
...
<!-- REAL NUMBERS (updated post-JD-fix 2026-08-19 — from `git diff origin/main...HEAD --numstat`):
  NuevaCategoriaForm.tsx        +140 −0   (production component)
  NuevaCategoriaForm.spec.tsx   +331 −0   (12 tests: 9 original + Cancelar + whitespace-only + double-submit; retry test rebuilt on a deferred promise)
  CategoriasPanel.spec.tsx      +76  −2   (T5c.3 toggle test + close-on-success integration test + imports + mock ref)
  CategoriasPanel.tsx           +16  −7   (NuevaCategoriaForm wiring, replacing placeholder)
  configuracion.tsx             +4   −1   (onCatalogoChange wiring)
  tasks.md                      +25  −4   (REAL NUMBERS updates)
  Total ledger-scope insertions: 592 lines / 15 deletions (forecast ~210; overrun is spec density —
  the JD fix round added Cancelar, whitespace-only nombre, double-submit-disabled and the
  close-on-success integration test on top of the original apply).
  14 net new tests: 12 in NuevaCategoriaForm.spec.tsx + 2 new in CategoriasPanel.spec.tsx (15 there total).
...
- [x] **T6a.4 (GREEN)** Create `apps/mobile/src/components/configuracion/EditarCategoria.tsx` —
      ...
      (`Guardar`/`Cancelar`/`Eliminar categoría`); bucket-dirty Guardar is an early-return
      (Alert.alert confirmation wired in PR6b); Eliminar calls `eliminarCategoria` directly —
      PR6b wraps it in the Alert.alert confirmation flow (MCTG-05);
      `PatronesSection` renders a placeholder until PR7.
      - Verify: `pnpm --filter @moneydiary/mobile test EditarCategoria.spec.tsx` — 13 green.
...
5. EditarCategoria.tsx PR6a: bucket-dirty Guardar is an early-return stub; Eliminar calls eliminarCategoria directly (no Alert.alert yet — PR6b wraps it). This is the correct slice boundary per design §5.

Deviations from design:
- EditarCategoria's Eliminar in PR6a calls eliminarCategoria directly (no Alert.alert) — PR6b adds the Alert.alert wrapper. The button is "present" (class 3 satisfied) and the route uses it; PR6b changes the call site to add the confirmation flow.
- ActualizarCategoria bucket-dirty case: in PR6a Guardar is an early-return. The test pins this with a "NOT called" assertion (fix 11). PR6b's positive case will flip this expectation when the Alert.alert flow is wired. -->
