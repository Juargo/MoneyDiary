...
**Migración backend NestJS → Express (ADR-028) — ✅ CERRADA, MERGEADA A `main` Y DEPLOYADA (2026-07-24, PR #109).** 10 slices TDD + node-cron; `domain`/`application` sin tocar (ADR-005). 8d verificado por smoke-test del entrypoint de prod (`start:prod`: boot + matriz curl 200/401/401/401 + conectividad DB vía login→401). **Deuda:** correr los e2e/int con DB (varios bit-rotteados de sesión desde Sprint 6) + reubicar los sobrevivientes framework-agnósticos de `http/` (cosmético) + flake intermitente de supertest a vigilar en CI. Detalle slice por slice en `openspec/changes/migrate-api-to-express/`.
