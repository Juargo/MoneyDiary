...
**Migración backend NestJS → Express (ADR-028) — ✅ CERRADA, MERGEADA A `main` Y DEPLOYADA (2026-07-24, PR #109).** 10 slices TDD + node-cron; `domain`/`application` sin tocar (ADR-005). 8d verificado por smoke-test del entrypoint de prod (`start:prod`: boot + matriz curl 200/401/401/401 + conectividad DB vía login→401). **Deuda:** correr los e2e/int con DB — **bloqueado por infra**: el `.env` apunta a prod y el gate `db-safety` los rechaza (correcto, es destructivo); necesitan una **DB de dev/staging separada** (Postgres local o branch Supabase). Además varios e2e están bit-rotteados de sesión desde Sprint 6. También: reubicar los sobrevivientes framework-agnósticos de `http/` (cosmético). Detalle slice por slice en `openspec/changes/migrate-api-to-express/`.

**En vuelo ahora (2026-07-24):** el mobile de Sprint 8 (US-033, PRs #76/#77) en espera del gate Maestro. Sprint 9 y las slices web de Sprint 8 ya están en `main`.
