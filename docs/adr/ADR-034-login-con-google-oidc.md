...
## Amendment (US-041, 2026-08-13)

US-041 agrega la vinculación/desvinculación explícita de una identidad de Google desde el perfil autenticado (`POST /api/perfil/google/vincular`, `POST /api/perfil/google/desvincular`) — no reemplaza nada de lo decidido arriba; el texto original queda intacto como registro de lo que se decidió y cuándo.

1. **Dos caminos de vinculación.** El implícito (login con Google, sin cambios: `email_verified` + match por `emailBlindIndex`) convive ahora con uno explícito (vinculación desde el perfil, userId-bound, con re-verificación de la password actual). El gate de `email_verified` pertenece SOLO al camino implícito — el camino explícito nunca consulta el email de Google en absoluto, así que no hay nada que ese gate pudiera proteger ahí. La desvinculación explícita exige el mismo control: `POST /api/perfil/google/desvincular` también re-verifica la password actual (`passwordActual`) antes de tocar `googleSub` — un control de aplicación distinto del invariante de presencia de `passwordHash` a nivel de `WHERE` descrito en el punto 4 (ese protege la fila incluso si este re-check de aplicación se rompiera).
2. **Callback de doble propósito.** `GET /api/auth/google/callback` sigue siendo UNA sola ruta, ahora distinguiendo login de vinculación explícita mediante un marcador firmado con HMAC (`link`) dentro de `md_oauth` — no una segunda redirect URI. El texto original de `oauth-transient-cookie.ts` razona que `state`/`nonce`/`codeVerifier` **"siguen sin firmar ni cifrar"** porque "un atacante que puede setear una cookie en el dominio no necesita forjar contenido, puede obtener una válida iniciando su propio flujo" — ese razonamiento deja de aplicar al campo `link`: es el ÚNICO campo del payload que NOMBRA UNA CUENTA (`userId`), y el mismo atacante que puede setear su propia cookie solo obtiene un `state` válido para SU PROPIA sesión, nunca un `link.userId` apuntando a la víctima. Por eso `link.mac` sí lleva integridad (HMAC-SHA256, `link-intent.ts`), mientras el resto del payload sigue sin firma.
3. **La regla ★ de no re-link es un invariante a nivel de base de datos, no de un único use case.** Se aplica en DOS call sites — `VincularGoogleUseCase` (chequeo de negocio) y la restricción `googleSub @unique` de Postgres (segunda línea de defensa, incondicional: colapsa cualquier colisión concurrente a `false`, nunca una excepción sin manejar) — precisamente porque ningún único punto de aplicación en código alcanza a cubrir una carrera entre dos requests concurrentes.
4. **Nuevo invariante de cuenta, de primera clase: una cuenta nunca queda sin método de acceso.** `googleSub` solo puede limpiarse mientras `passwordHash IS NOT NULL` — aplicado en el `WHERE` de la escritura condicional de desvinculación (`updateMany({ where: { id, passwordHash: { not: null }, googleSub: { not: null } }, data: { googleSub: null } })`), no en un pre-check de aplicación que dejaría una ventana de carrera entre leer y escribir.
