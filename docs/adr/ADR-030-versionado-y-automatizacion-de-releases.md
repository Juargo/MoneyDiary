...
✅ **Implementado** — change SDD `versioning-release-automation` mergeado a `main` (2026-07-27) vía **4 PRs encadenados**: #118 (Slice A — enforcement de ADR-020 Git Hooks Husky Monorepo, precondición de este ADR) · #119 (Slice B — bootstrap de release-please en manifest mode) · #120 (Slice C — split de CI por path) · #121 (Slice D — CD híbrido). **Pendiente:** activación en plataformas (secret `EXPO_TOKEN`, sincronizar `buildFilter` en Render). **Protección de rama en `main` (ítem C.7) ✅ activa** (verificado 2026-09-14): PR obligatorio, checks `CI success` y `Commitlint` requeridos con la rama al día respecto de `main`, `enforce_admins` activo, sin force-push ni borrado. **Vercel (2026-09-14):** Root Directory confirmado (`apps/web`/`apps/landing`, verificado en los logs de build) y Deep Clone deja de ser precondición: el `ignoreCommand` pasó de `git diff HEAD^ HEAD` a `scripts/vercel-ignore-build.sh` (#673), que en preview trae `main` y compara la rama contra su merge-base, y en producción usa `HEAD^`, que cabe en el clon de profundidad 10 que hace Vercel por defecto. **Follow-up abierto:** auditar el gate de ADR-021 Análisis de Seguridad en el Pipeline contra el CI ya partido por path.
